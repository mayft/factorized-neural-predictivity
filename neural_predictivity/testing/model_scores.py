...
class MyBenchmark(Benchmark):
    def __init__(self,identifier,metric,assembly,region,visual_degrees=8,num_trials=1,consistent=None,pcs=False,n=None):
        ...
        if metric == 'pls_cv':
            self._metric = CrossRegressedCorrelation(regression=pls_regression(regression_kwargs={'n_components':n}),correlation=pearsonr_correlation(),crossvalidation_kwargs=cvk)
...
if __name__ == "__main__":
    ...
    for name, model in models.items():
        ...
        for metric in metrics:    
            comps = [25] if metric == 'pls_cv' else [None]
            for n in comps:
                prediction_scores = {}
                for region, dataset in regions.items():
                    print (f'scoring {name} on {region} data with {metric} {n or ""}')
                    benchmark = MyBenchmark(identifier='benchmark', assembly=dataset, region=region,metric=metric,consistent=consistent_pcs[region],pcs=True,n=n)
                    prediction_scores[region] = model_scorer(benchmark,model.layers)

                get_layer_scores(prediction_scores).to_csv(f'neural_prediction_results/{model.identifier}_{metric}_{n or ""}_pca.csv')
